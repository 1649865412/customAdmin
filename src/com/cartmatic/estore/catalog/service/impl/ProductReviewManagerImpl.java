...
public class ProductReviewManagerImpl extends GenericManagerImpl<ProductReview> implements ProductReviewManager {
	...
	public void save(ProductReview entity) {
		...
		//添加对产品的整体评价到产品
		//TODO 通过对状态来判断是否处理，考虑添加字段记录&&entity.getOrigStatus().intValue()==Constants.STATUS_INACTIVE.intValue()
		if(entity.getId()!=null&&entity.getStatus()!=null&&entity.getStatus().intValue()==Constants.STATUS_ACTIVE.intValue()&&entity.getRate().intValue()>0){
			updateProductReviewStat(entity.getStoreId(),entity.getProductId());
		}
	}
